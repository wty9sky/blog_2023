    ...
    <div class="mx-auto bg-white dark:bg-gray-900">
      <div class="relative">
        <div class="mx-auto lg:max-w-5xl">
          ...
          <div v-for="(wrap, index) in props.projects" :key="index">
            <span
              class="mt-[20px] mb-[10px] font-bold text-2xl text-center block"
              >{{ wrap.label }}</span
            >
              <div class="my-[20px]" v-for="(info, index) in wrap.list" :key="index">
                <span
                  style="text-decoration: none"
                  class="text-base no-underline font-bold block text-center"
                  >{{ info.label }}</span
                >
