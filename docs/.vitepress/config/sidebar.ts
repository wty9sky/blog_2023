...
function getProjectItems(path: string) {
  // 侧边栏分组数组
  let groups: DefaultTheme.SidebarItem[] = [
    // {
    //   text: `开发日志`,
  ...
  //     link: `/${path}/开发日志/${article.name.replace('.md', '')}`,
  //   });
  //   total += 1;
  // })
  ...
  // groups[0].collapsed = items.length < groupCollapsedSize || total > titleCollapsedSize
